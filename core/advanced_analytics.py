...
def identify_strategy_fit(df: pl.DataFrame) -> pl.DataFrame:
    ...
    - INSTANT_FLIP: Ultra-high volume, tight spreads, flip same day
    - SHORT_HOLD: Moderate volume, decent margins, 1-2 day hold
    - SWING: High margin, moderate volume, hold 3-7 days
    - WHALE: 1M+ profit per item, very expensive items (true whale territory)
    """
    df = df.with_columns([
        # INSTANT_FLIP: Very high volume + tight spreads = quick flips
        pl.when(
            (pl.col("hourly_volume") >= 5000) & 
            (pl.col("spread_pct") <= 5.0) &
            (pl.col("net_edge") >= 50)
        )
        .then(pl.lit("INSTANT_FLIP"))
        
        # WHALE: 1M+ profit per item, expensive items (5M+)
        .when(
            (pl.col("net_edge") >= 1_000_000) &
            (pl.col("mid_price") >= 5_000_000)
        )
        .then(pl.lit("WHALE"))
        
        # SWING: Good margins, willing to hold longer
        .when(
            (pl.col("edge_pct") >= 5.0) &
        ...
        # SHORT_HOLD: Middle ground - decent volume, decent margin
        .when(
            (pl.col("hourly_volume") >= 500) &
            (pl.col("edge_pct") >= 2.0)
        )
        .then(pl.lit("SHORT_HOLD"))
...
def calculate_execution_quality(df: pl.DataFrame) -> pl.DataFrame:
    """
    Simple execution quality check - can you actually fill this trade?
    Backend: complex logic. Frontend: simple confidence score.
    """
    df = df.with_columns([
        # Execution confidence (0-100): Will you fill at quoted prices?
    ...
    # Expected slippage (% worse than quoted - hidden from user, used in calcs)
    df = df.with_columns([
    ...
    # Realistic profit after slippage (backend calc, shown as adjusted profit)
    df = df.with_columns([
        (pl.col("net_edge") * (1 - pl.col("slippage_pct") / 100)).cast(pl.Int64).alias("realistic_profit")
    ])
    
    return df


def detect_manipulation_simple(df: pl.DataFrame) -> pl.DataFrame:
    """
    Flag obvious manipulation - keep it simple
    """
    df = df.with_columns([
    ...
    # Simple manipulation flag
    df = df.with_columns([
        pl.when(
            (pl.col("volume_imbalance") > 0.9) &
...
def calculate_simple_downside(df: pl.DataFrame) -> pl.DataFrame:
    """
    Estimate max realistic loss if trade goes bad
    Simple for user, but important for safety
    """
    df = df.with_columns([
        # Max downside % based on liquidity
        pl.when(pl.col("hourly_volume") >= 5000)
        .then(pl.lit(5))
        .when(pl.col("hourly_volume") >= 1000)
        .then(pl.lit(8))
...
def build_advanced_analytics(df: pl.DataFrame, total_capital: int = 50_000_000) -> pl.DataFrame:
    ...
    10. Execution quality (NEW - realistic fills)
    11. Manipulation detection (NEW - avoid traps)
    12. Downside protection (NEW - max loss)
    """
    ...
    # NEW: Reality-based metrics (complex backend, simple frontend)
    df = calculate_execution_quality(df)
    df = detect_manipulation_simple(df)
    df = calculate_simple_downside(df)
