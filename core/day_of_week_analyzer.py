...
def analyze_day_of_week_patterns(timeseries_df: pl.DataFrame) -> Dict:
    ...
    if len(day_stats) < 3:  # Need at least 3 days of data
        return {
            'has_data': False,
            'best_buy_day': 'Unknown',
            'best_sell_day': 'Unknown',
    ...
    # Find cheapest day to buy (lowest avgLowPrice)
    best_buy_idx = day_stats['avg_buy_price'].arg_min()
    best_buy_day_num = day_stats['day_of_week'][best_buy_idx]
    best_buy_price = int(day_stats['avg_buy_price'][best_buy_idx])
    
    # Find most expensive day to sell (highest avgHighPrice)
    best_sell_idx = day_stats['avg_sell_price'].arg_max()
    best_sell_day_num = day_stats['day_of_week'][best_sell_idx]
    best_sell_price = int(day_stats['avg_sell_price'][best_sell_idx])
    ...
    weekend_avg = day_stats.filter(pl.col('day_of_week').is_in([5, 6]))['avg_sell_price'].mean() if len(day_stats.filter(pl.col('day_of_week').is_in([5, 6]))) > 0 else 0
    weekday_avg = day_stats.filter(~pl.col('day_of_week').is_in([5, 6]))['avg_sell_price'].mean() if len(day_stats.filter(~pl.col('day_of_week').is_in([5, 6]))) > 0 else 0
